...
try:
    ...
    # Ela ignora os valores extremos. Não sofre a interferência dos valores extremos.
    iqr = q3 - q1
    ...
    # Descrevem a distribuição dos dados, sem a necessidade de uma 
    # visualização gráfica.
    # Quanto uma distribuição tende para os valores maiores ou menores
    # Tem como objetivo quantificar a magnitude da influência dos dados
    # extremos (menores e maiores) na distribuição.
    # Valor da Skewness	 - Tipo de Assimetria  - Interpretação
    # -0.5 a 0.5 -	Distribuição Simétrica	- Os dados estão bem distribuídos
    # em torno da média.
    
    # Simetria, Assimetria Positiva ou Negativa
    # 0.5 a 1.0	 - Assimetria Moderada Positiva	Valores altos puxam a média para cima (média > mediana).
    # > 1.0	 - Assimetria Alta Positiva - Forte concentração de dados menores
    # com valores extremos altos.
    # -0.5 a -1.0	Assimetria Moderada Negativa  -  Valores baixos puxam a 
    # média para baixo (média < mediana).
    # < -1.0	Assimetria Alta Negativa	Forte concentração de dados maiores 
    # com valores extremos baixos.
    assimetria = df_roubo_veiculo['roubo_veiculo'].skew()

    # curtpse. Kurtosis
    # Medida que apresenta dois pontos:
    #   O quanto os dados, se aproximam da média;
    #   O “comportamento das caudas” (dados não concentrados).
    #   
    #   < 2.5: Platicúrtica - Dados mais dispersos em relação a média (outliers comuns)
    #   
    #   >= 2.5 e <= 3.5: Mesocúrtica - Distribuição dos dados tende 
    #   a ser normal "Uniforme" em torno da média, 
    #   
    #   Outliers são um pouco menos comuns (Exceto em BigData).
    #   > 3.5: Leptocúrtica - Dados extremamente concentrados entorno da média - Outliers são muito comuns.
...
# Explicar a Curva da Assimeria e Curtose
try:
    # plotar a curva de densidade
    sns.kdeplot(df_roubo_veiculo["roubo_veiculo"], bw_adjust=3, color="red")
    plt.show()
except Exception as e:
...
try:
    ...
    plt.text(0.5, 0.2, f'Assimetria: {assimetria:.4f}', fontsize=10)
    plt.text(0.5, 0.1, f'Curtose: {curtose:.4f}', fontsize=10)
    ...
    plt.hist(array_roubo_veiculo, bins=77, edgecolor='black', alpha=0.7) # bins = largura da barra do histograma intervalo ou categorização da coluna => 385315 / 77 = 5.004,12987
